...
from utils import cleanup_temp_file, firefox_time_to_datetime, safe_copy_database
...
def read_profile_history(
    ...
    if not profile.history_path.exists():
        raise ProfileReadError(profile.label, "places.sqlite database not found (skipped).")
    ...
    try:
        ...
        for title, url, visit_count, prtime in rows:
            visit_time = firefox_time_to_datetime(prtime)
